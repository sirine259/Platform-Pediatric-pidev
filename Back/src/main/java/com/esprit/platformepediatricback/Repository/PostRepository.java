...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query("SELECT p FROM Post p WHERE p.isDeleted = false AND " +
           ...
           "LOWER(p.content) LIKE LOWER(CONCAT('%', :keyword, '%')))")
    Page<Post> searchPostsPaginated(@Param("keyword") String keyword, Pageable pageable);
    
    @Query("SELECT p FROM Post p WHERE p.forum.id = :forumId AND p.isDeleted = false ORDER BY p.createdAt DESC")
    Page<Post> findByForumIdPaginated(@Param("forumId") Long forumId, Pageable pageable);
}
